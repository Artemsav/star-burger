...
Адресс вебсайта: [Star-burger](https://artemsav.store)
...
### Установка системы логирования (Rollbar)

Для работы системы логирования необходимо зарегистрироваться на сайте [Rollbar](https://rollbar.com/), добавить в переменные окружения две переменные:

 `ROLLBAR_TOKEN` — токен, которые получите [здесь](https://rollbar.com/)

 `ROLLBAR_ENV` — окружение в котором запущен сайт (разработка / продакш)
